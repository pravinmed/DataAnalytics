# -*- coding: utf-8 -*-
"""
Created on Sat Sep  5 16:05:19 2015
...
def mapper():
    ...
    for line in sys.stdin:
        ...
        if len(words) >=22:
            entry = float(words[6].strip())
            rainOrFog = format_key(int(float(words[14].strip())),int(float(words[15].strip())))
            output='{0}\t{1}'.format(rainOrFog,entry)
            print output
            logging.info(output)
